...
import { resolvePartyInvitationLocale } from '@/i18n/party';
...
const defaultState: EngineState = {
  ...
  mode: 'standard', invitationLocale: 'ar', weddingGuest: defaultWeddingGuest,
...
function GuestPage() {
  ...
  />;
  return <div className="grain min-h-[100dvh] overflow-hidden bg-[#FAF7F2] text-[#2D2421]" lang={state.invitationLocale} dir={localeDirection(state.invitationLocale)}>
    ...
    <main className="relative z-10 mx-auto max-w-4xl px-5 pb-28 sm:px-8">
      ...
      <AnimatePresence mode="wait">
        {state.rsvp === 'pending' && <motion.div key="rsvp" initial={{ opacity: 0, scale: .98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="suite-card mx-auto max-w-xl p-7 text-center sm:p-12">
          <Eyebrow>{invitationT(state.invitationLocale, 'privateInvitation')} HASHIM</Eyebrow>
...
function PartyStudioPage({ embedded = false }: { embedded?: boolean }) {
  const { state, ready, toggleBlock, reorderBlocks, updateBlock, setMode, setInvitationLocale } = useEngine();
  const { t } = useAppLocale();
        ...
        <div className="mt-8 grid grid-cols-[minmax(0,1fr)] gap-8 lg:grid-cols-[minmax(0,1fr)_340px]">
          <div className="min-w-0 space-y-8">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              <StudioStat label={t('invitationStatus')} value={t('live')} detail={t('privateLinkActive')} icon={Sparkles} />
              <StudioStat label={t('responses')} value={accepted ? '1 / 1' : '0 / 1'} detail={accepted ? t('checkedIn') : t('awaitingReply')} icon={ClipboardCheck} />
              <StudioStat label={t('doorPass')} value={t(state.checkedIn ? 'used' : 'ready')} detail={t(state.checkedIn ? 'checkedIn' : 'webQrEnabled')} icon={TicketCheck} />
            </div>
              ...
              <Reorder.Group axis="y" values={state.blocks} onReorder={reorderBlocks} className="mt-7 space-y-3">
                {state.blocks.map((block) => (
                  <Reorder.Item key={block.key} value={block} className={`flex items-center gap-3 rounded-2xl border p-3 transition ${block.enabled ? 'border-[#D4AF37]/55 bg-[#FFFDF9]/50' : 'border-[#2D2421]/10 bg-[#2D2421]/[.03] opacity-55'}`}>
                    ...
                    <button onClick={() => setActiveEditor(block.key)} data-testid={`button-edit-${block.key}`} aria-label={`${t('edit')} ${t(block.key)}`} className="focus-ring rounded-full border border-[#D4AF37]/55 p-2 text-[#0A2E23] hover:bg-[#D4AF37]/10">
                      <Edit3 size={14} />
                    </button>
                    <button onClick={() => toggleBlock(block.key)} data-testid={`button-toggle-${block.key}`} aria-label={`${t(block.enabled ? 'hide' : 'show')} ${t(block.key)}`} className={`focus-ring relative h-6 w-11 rounded-full transition ${block.enabled ? 'bg-[#0A2E23]' : 'bg-[#2D2421]/20'}`}>
            ...
            <AnimatePresence mode="wait">
              {activeBlock ? (
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass-panel rounded-[28px] p-6">
                  ...
                  <p className="mt-4 font-display text-2xl text-[#0A2E23]">{t('makePersonal')}</p>
                  <p className="mt-2 text-xs leading-5 text-[#2D2421]/60">{t('makePersonalHelp')}</p>
                </motion.div>
              )}
            </AnimatePresence>
          </aside>
