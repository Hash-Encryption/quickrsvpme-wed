...
const defaultState: EngineState = { rsvp: 'pending', plusOnes: 0, song: '', meal: '', checkedIn: false, blocks: initialBlocks };
...
function EngineProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (raw) {
      try { setState({ ...defaultState, ...JSON.parse(raw) }); } catch { setState(defaultState); }
    }
    setReady(true);
  }, []);
  useEffect(() => {
...
function GuestPage() {
  const { state, ready, setRsvp, setSong, setMeal } = useEngine();
  const { token } = useParams<{ token: string }>();
  const [openFaq, setOpenFaq] = useState<number | null>(0);
  const validToken = token === 'demo' || token === 'k82f9x';
...
function StudioPage() {
  const { state, ready, toggleBlock, reorderBlocks, updateBlock } = useEngine();
  ...
  return <div className="grain min-h-[100dvh] bg-[#FAF7F2] text-[#2D2421]"><div className="gold-thread" /><QuietHeader studio />
    <main className="relative z-10 mx-auto max-w-7xl px-5 pb-20 sm:px-8 lg:px-14">
      <FadeIn><div className="flex flex-col justify-between gap-7 border-b border-[#D4AF37]/35 pb-8 md:flex-row md:items-end"><div><Eyebrow>HOST STUDIO / MAYA &amp; LIAM</Eyebrow><h1 className="mt-3 font-display text-6xl leading-[.82] text-[#0A2E23] sm:text-7xl">Your suite,<br /><span className="text-[#D4AF37]">in your hands.</span></h1><p className="mt-5 max-w-lg text-sm leading-6 text-[#2D2421]/65">Shape the guest experience, then send a private link that feels like yours.</p></div><div className="flex gap-2"><Link href="/i/demo" data-testid="link-preview-invitation" className="focus-ring inline-flex items-center gap-2 rounded-full border border-[#D4AF37]/70 px-4 py-3 text-[10px] font-bold uppercase tracking-[.12em] text-[#0A2E23]"><ExternalLink size={14} /> Preview</Link><Link href="/scanner" data-testid="link-open-scanner" className="focus-ring inline-flex items-center gap-2 rounded-full bg-[#0A2E23] px-4 py-3 text-[10px] font-bold uppercase tracking-[.12em] text-[#FFFDF9]"><QrCode size={14} /> Door scanner</Link></div></div></FadeIn>
...
function GuestManager() {
  ...
  const sendWhatsApp = () => { const message = encodeURIComponent('Maya & Liam would love to celebrate with you. Your private invitation: https://suite.maya-liam.com/i/k82f9x'); window.open(`https://wa.me/?text=${message}`, '_blank'); };
  const exportGuests = () => { const csv = 'Guest,Plus ones,RSVP,Checked in\\nHashim Alnimari,0,' + state.rsvp + ',' + state.checkedIn + '\\n'; const url = URL.createObjectURL(new Blob([csv], { type: 'text/csv' })); const link = document.createElement('a'); link.href = url; link.download = 'maya-liam-caterer-list.csv'; link.click(); URL.revokeObjectURL(url); };
  return <div className="suite-card overflow-hidden p-6 sm:p-8"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><Eyebrow>GUEST LIST / 01</Eyebrow><h2 className="mt-2 font-display text-4xl text-[#0A2E23]">The people who matter.</h2></div><Button variant="ivory" icon={ArrowDownToLine} onClick={exportGuests}>Caterer export</Button></div><div className="mt-6 overflow-x-auto"><table className="w-full min-w-[590px] text-left"><thead><tr className="border-b border-[#D4AF37]/35 text-[10px] uppercase tracking-[.12em] text-[#2D2421]/50"><th className="pb-3 font-semibold">Guest</th><th className="pb-3 font-semibold">Party</th><th className="pb-3 font-semibold">Response</th><th className="pb-3 text-right font-semibold">Invite</th></tr></thead><tbody><tr className="border-b border-[#D4AF37]/20"><td className="py-4"><div className="flex items-center gap-3"><InitialsAvatar /><div><p className="text-sm font-semibold text-[#0A2E23]" data-testid="row-guest-hashim">Hashim Alnimari</p><p className="text-[10px] text-[#2D2421]/50">k82f9x</p></div></div></td><td className="py-4 text-sm">1 guest</td><td className="py-4"><span className={`rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-[.1em] ${state.rsvp === 'accepted' ? 'bg-[#0A2E23]/10 text-[#0A2E23]' : 'bg-[#D4AF37]/15 text-[#8A6712]'}`}>{state.rsvp}</span></td><td className="py-4 text-right"><button onClick={sendWhatsApp} data-testid="button-whatsapp-hashim" className="focus-ring inline-flex items-center gap-2 rounded-full border border-[#D4AF37]/60 px-3 py-2 text-[10px] font-bold uppercase tracking-[.08em] text-[#0A2E23] hover:bg-[#D4AF37]/10"><MessageCircle size={14} /> WhatsApp</button></td></tr></tbody></table></div><p className="mt-5 flex items-center gap-2 text-[10px] text-[#2D2421]/50"><Link2 size={12} className="text-[#A98219]" /> Personal link ready · /i/k82f9x</p></div>;
}
