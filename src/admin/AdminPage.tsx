...
export function AdminPage({ section, events, summary, saveStatus, storageError }: { section: AdminSection; events: AdminEventRecord[]; summary: AdminSummary; saveStatus: string; storageError: string }) {
...
function Events({ events }: { events: AdminEventRecord[] }) {
  const { t } = useAppLocale();
  return <div className="grid gap-3">{events.map((event) => <article key={`${event.type}-${event.id}`} className="rounded-2xl border border-[#D9D2C5] bg-white p-5 sm:flex sm:items-center sm:gap-5"><CalendarDays className="shrink-0 text-[#A4813C]" /><div className="mt-3 min-w-0 flex-1 sm:mt-0"><p className="text-[10px] font-bold uppercase tracking-[.12em] text-[#8B7040]">{t(event.type)}</p><h2 className="truncate font-semibold">{event.name}</h2><p className="mt-1 text-xs text-[#756F66]"><bdi>{event.date} · {event.venue}</bdi></p></div><div className="mt-4 flex gap-4 text-xs text-[#756F66] sm:mt-0"><span>{event.guests} {t('guests')}</span><span>{event.checkedIn} {t('checkedIn')}</span></div><Link href={buildProjectRoute(event.type, event.id, 'overview')} className="focus-ring mt-4 inline-flex min-h-11 items-center rounded-full border border-[#D9D2C5] px-4 text-xs font-semibold sm:mt-0">{t('overview')}</Link></article>)}</div>;
}
...
function Support({ locale, dir, projects, saveStatus, storageError }: { locale: string; dir: string; projects: number; saveStatus: string; storageError: string }) {
  const { t } = useAppLocale();
  const rows = [[t('appLanguage'), locale], [t('direction'), dir], [t('events'), String(projects)], [t('weddingWorkspace'), saveStatus], [t('browserPersistence'), 'IndexedDB + localStorage'], [t('publicRoute'), '/i/:token'], [t('scanner'), t('manualLocalOnly')]];
