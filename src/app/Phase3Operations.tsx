...
export function BackendGuestManager({ project }: { project: ProjectSummary }) {
  ...
  return <div>
    ...
    <section className="mt-6 rounded-3xl border border-[#D9D2C5] bg-white p-5 sm:p-6"><div className="flex items-center justify-between gap-3"><div><h2 className="text-lg font-semibold">{t('generalRequests')}</h2><p className="mt-1 text-xs text-[#756F66]">{t('generalRequestsHelp')}</p></div><span className="rounded-full bg-[#F0E9DB] px-3 py-2 text-[10px] font-bold">{requests.filter((request) => request.state === 'awaiting').length} {t('awaiting')}</span></div>{requests.length === 0 ? <p className="mt-5 text-sm text-[#756F66]">{t('noGeneralRequests')}</p> : <div className="mt-5 grid gap-3">{requests.map((request) => <article key={request.id} className="flex flex-col gap-4 rounded-2xl border border-[#E6DFD3] p-4 sm:flex-row sm:items-center"><div className="min-w-0 flex-1"><p className="font-semibold">{request.name}</p><p className="mt-1 text-xs text-[#756F66]"><bdi>{request.phone}</bdi> · <time dateTime={request.created_at}>{new Date(request.created_at).toLocaleString()}</time></p></div><span className="w-fit rounded-full bg-[#F5F2EC] px-3 py-2 text-[10px] font-bold uppercase">{t(request.state)}</span>{request.state === 'awaiting' && <div className="flex gap-2"><button disabled={busy === request.id} onClick={() => void review(request, 'approved')} className="min-h-11 rounded-xl bg-[#0C2D24] px-4 text-xs font-bold text-white transition hover:bg-[#174839] disabled:opacity-40">{t('approve')}</button><button disabled={busy === request.id} onClick={() => void review(request, 'rejected')} className="min-h-11 rounded-xl border border-[#8c302b]/40 px-4 text-xs font-bold text-[#8c302b] transition hover:bg-[#8c302b]/10 disabled:opacity-40">{t('reject')}</button></div>}</article>)}</div>}</section>
    <div className="mt-6 grid gap-2 rounded-2xl border border-[#D9D2C5] bg-white p-4 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_110px_auto]"><input value={newGuest.name} onChange={(event) => setNewGuest({ ...newGuest, name: event.target.value })} placeholder={t('guest')} className="qr-field-inline min-h-11 rounded-xl px-3 text-sm" /><input value={newGuest.phone} onChange={(event) => setNewGuest({ ...newGuest, phone: event.target.value })} placeholder={t('missingPhone')} className="qr-field-inline min-h-11 rounded-xl px-3 text-sm" /><input type="number" min="0" max="50" value={newGuest.companions} onChange={(event) => setNewGuest({ ...newGuest, companions: Number(event.target.value) })} aria-label={t('companionAllowance')} className="qr-field-inline min-h-11 rounded-xl px-3 text-sm" /><button disabled={!newGuest.name.trim() || busy === 'create'} onClick={() => { setBusy('create'); void createGuest(project.id, newGuest.name.trim(), newGuest.phone.trim(), newGuest.companions).then(() => { setNewGuest({ name: '', phone: '', companions: 0 }); return refresh(); }).catch(() => setError(t('operationFailed'))).finally(() => setBusy('')); }} className="min-h-11 rounded-xl bg-[#0C2D24] px-5 text-xs font-bold text-white transition hover:bg-[#174839] disabled:opacity-40">{t('addGuest')}</button></div>
    <div className="mt-6 grid gap-2 sm:grid-cols-[minmax(0,1fr)_220px]"><label className="sr-only" htmlFor="phase3-guest-search">{t('searchGuests')}</label><input id="phase3-guest-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t('searchGuests')} className="qr-field-inline min-h-12 rounded-xl px-4 text-sm" /><label className="sr-only" htmlFor="phase3-guest-filter">{t('filter')}</label><select id="phase3-guest-filter" value={filter} onChange={(event) => setFilter(event.target.value as GuestFilter)} className="qr-field-inline min-h-12 rounded-xl px-4 text-sm font-medium"><option value="all">{t('allResponses')}</option><option value="not_opened">{t('notOpened')}</option><option value="opened_no_rsvp">{t('openedNoRsvp')}</option><option value="accepted">{t('accepted')}</option><option value="declined">{t('declined')}</option><option value="pending">{t('pending')}</option><option value="not_arrived">{t('notArrived')}</option><option value="partial">{t('partiallyCheckedIn')}</option><option value="complete">{t('fullyCheckedIn')}</option></select></div>
