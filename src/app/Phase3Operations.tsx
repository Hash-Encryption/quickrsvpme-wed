...
import { Check, CircleAlert, Clock3, QrCode, Search, Users, XCircle } from 'lucide-react';
...
export function BackendGuestManager({ project }: { project: ProjectSummary }) {
  ...
  return <div>
    ...
    {visible.length === 0 ? <div className="mt-6 rounded-3xl border border-dashed border-[#C8BCA8] bg-white p-10 text-center text-sm text-[#756F66]">{t('noGuests')}</div> : <div className="mt-6 grid gap-3">{visible.map((guest) => {
      ...
      const guestTags = guest.event_guest_tag_assignments?.flatMap((item) => item.event_guest_tags.map((tag) => tag.name)) ?? [];
...
export function BackendScanner({ project }: { project: ProjectSummary }) {
  ...
  const verify = async () => { setBusy(true); setError(''); try { const next = await resolveCheckin(value, project.id); setResult(next); setArriving(Math.min(1, next.remaining_expected ?? 1)); } catch { setError(t('networkError')); } finally { setBusy(false); } };
  ...
  const reset = () => { setValue(''); setResult(null); setError(''); setArriving(1); requestAnimationFrame(() => inputRef.current?.focus()); };
