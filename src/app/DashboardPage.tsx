...
export function DashboardPage({ projects, account, onSignOut, onCreate, onRename, onArchive, onDelete }: { projects: DashboardProject[]; account: DashboardAccount; onSignOut: () => void; onCreate: (type: ProjectSummary['type'], title: string) => Promise<void>; onRename: (id: string, title: string) => Promise<void>; onArchive: (id: string) => Promise<void>; onDelete: (id: string) => Promise<void> }) {
  const { t, dir } = useAppLocale();
  const [type, setType] = useState<ProjectSummary['type']>('wedding');
  const [title, setTitle] = useState('');
  const [names, setNames] = useState<Record<string, string>>({});
  const [busy, setBusy] = useState('');
  const run = async (key: string, operation: Promise<void>) => { setBusy(key); try { await operation; } finally { setBusy(''); } };
